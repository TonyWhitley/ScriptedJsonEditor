    ...
    def __init__(self, versionStr=''):
        ...
        try:
            self.parser = argparse.ArgumentParser(
                prog=versionStr, description='JSON file editor')
...
%s
        """ % (sys.argv[0], JOBS_FILE_HELP_STR, JOB_DEFINITIONS_FILE_HELP_STR))
            ...
            self.parser.add_argument(
                '-p', '--player', help='Optional: player ID (default "player")')
        ...
        if self.args.filename:
            if self.args.filename[-5:].lower() == '.json':
                ...
                self.jobs_file = input(
                    'Enter jobs file name (just Enter to quit): ')
        else:
