...
class Tab:
    ...
    def __init__(self, parentFrame, menu2tab, goCommand=False):
        ...
        menu2tab.setJobDefinitionsFolderRefresh(
...
')
        ...
        self.tkLabelframe_jobSettings = tk.LabelFrame(
            parentFrame, text='Job settings')
        self.tkLabelframe_jobSettings.grid(row=3, pady=5, padx=5, ipadx=10)

        self.jobDefinitionFrames = self.JobDefinitionFrames(
            self.tkLabelframe_jobSettings, self.menu2tab)
    ...
    def fillJobDropdown(self, parentFrame):
        ...
        if choices != {}:
            popupMenu = tk.OptionMenu(
                self.jobFilesFrame, self.jobFileVar, *choices)
            tk.Label(
                self.jobFilesFrame,
                text="Choose a job file").grid(
                row=1,
                column=0,
                sticky='w')
            popupMenu.grid(row=1, column=1, ipadx=10, sticky='w')
            self.jobFileVar.trace('w', self.change_dropdown)
            # set the default option to the "first" dict item
            self.jobFileVar.set(next(iter(choices)))
        else:
    ...
    def change_dropdown(self, *args):
        ...
        # self.initialJobDefinitions = {} # for detecting if anything has
        # changed
        _jobsFile = os.path.join(
            self.menu2tab.jobsFolder,
            self.jobFileVar.get())
            ...
            for job in _jobs:
                for defnFile in job:
                    for defn in job[defnFile]:
                        self.jobDefinitionFrames.set_checkbutton(
                            defnFile, defn, 1)
    ...
    class JobDefinitionFrames:
        ...
        def __init__(self, parentFrame, menu2tab):
            all_job_definitions = get_all_job_definitions(
                menu2tab.jobDefinitionsFolder)
            ...
            if all_job_definitions:
                for job_definition_file_name, jobs in all_job_definitions.items():
                    ...
                    # for the frame
                    total_jobs += len(jobs.json_dict['job definitions']) + 1
                job_definition_file_names.sort(
                    key=self.__lenSecond, reverse=True)
            ...
            # Then display each job_definition_file in a frame in order of
            # number of jobs
            ...
            for job_definition_file_name, __ in job_definition_file_names:
                ...
                _tkLabelframe = tk.LabelFrame(
                    parentFrame, text=job_definition_file_name + '.json')
                self.checkbutton_IntVars[job_definition_file_name] = {}
        ...
        def set_checkbutton(self, job_definition_file_name, job_name, value):
            try:
                self.checkbutton_IntVars[job_definition_file_name][job_name].set(
                    value)
            except BaseException:
                print(
                    'Could not set job definition file "%s" job "%s" to "%s"' %
            ...
            for job_definition_file_name in self.checkbutton_IntVars:
                ...
                for job_name in self.checkbutton_IntVars[job_definition_file_name]:
                    if self.checkbutton_IntVars[job_definition_file_name][job_name].get(
                        ...
                        _jobs['jobs'][0][job_definition_file_name].append(
                            job_name)
                if _jobs['jobs'][0][job_definition_file_name] == []:
                    ...
                    _job_definition_file_names.append(
                        r'job_definitions\%s.json' %
                        job_definition_file_name)
...
if __name__ == '__main__':
    ...
    try:
        ...
        raise
    except BaseException:  # error raised as expected
        pass
